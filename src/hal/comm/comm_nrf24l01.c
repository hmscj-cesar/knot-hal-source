...
struct nrf24_data {
	...
	uint8_t write_offset; /*Writing offset handler*/
	uint8_t write_rt; /*Writing retry counter*/
...
static int write_raw(int spi_fd, int sockfd)
{
	...
	memcpy(
		opdu->payload,
		peers[sockfd-1].buffer_tx +
		peers[sockfd-1].write_offset,
		plen);
...
static int read_raw(int spi_fd)
{
	...
	while ((ilen = phy_read(spi_fd, &p, NRF24_MTU)) > 0) {
		...
		peers[p.pipe-1].keepalive_anchor = hal_time_ms();
